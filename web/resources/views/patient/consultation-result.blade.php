                ...
                <div class="p-6 text-gray-900">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-0.5">Daftar konsultasi yang telah selesai</p>
                        </div>
                    </div>
                    ...
                    @if($consultations->isEmpty())
                        <div class="text-center py-14">
                            ...
                            <p class="mt-3 text-sm text-gray-500">Belum ada hasil konsultasi yang tersedia.</p>
                        </div>
                    @else
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hasil Diagnosis</th>
                                    ...
                                    @foreach($consultations as $i => $consultation)
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-6 py-4 text-sm text-gray-900 max-w-xs">
                                                {{ $consultation->diagnosis->diagnosis_result ?? 'Tidak tersedia' }}
                                            </td>
                ...
                success: function(data) {
                    ...
                    if (data.diagnosis) {
                        ...
                        diagnosisSection += `
                            <div class="border-b pb-4">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-900">${data.diagnosis.result || 'Tidak tersedia'}</p>
                                </div>
                                ${data.diagnosis.notes ? `
                                <div class="mt-4">
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Catatan Dokter</p>
                                    <p class="mt-1 text-sm text-gray-900">${data.diagnosis.notes}</p>
                                </div>
                                ` : ''}
                                <div class="mt-4">
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Status Verifikasi</p>
                                    <span class="mt-1 inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${data.diagnosis.is_verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}">
                                        ${data.diagnosis.is_verified ? 'Terverifikasi' : 'Belum Terverifikasi'}
                                    </span>
                                </div>
                            </div>
                        `;
                        ...
                        if (data.diagnosis.processed_video_url || data.diagnosis.raw_video_url) {
                            ...
                            diagnosisSection += `
                                <div class="border-b pb-4">
                                    <h4 class="font-semibold text-gray-900 mb-3">Video Pemeriksaan</h4>
                        ...
                        if (data.diagnosis.images && data.diagnosis.images.length > 0) {
                            let imagesHtml = data.diagnosis.images.map(img => `
                                <div class="relative rounded-lg overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-all cursor-pointer" onclick="openLightbox('${img.image_url}')">
                                    ...
                                    ${img.ai_screening_result ? `
                                        <div class="p-2">
                                            <p class="text-[10px] text-gray-500 font-semibold uppercase">Hasil AI</p>
                            ...
                            diagnosisSection += `
                                <div class="border-b pb-4">
                                    <h4 class="font-semibold text-gray-900 mb-3">Foto Pemeriksaan <span class="text-xs text-gray-400 font-normal">(${data.diagnosis.images.length} foto)</span></h4>
                        ...
                        diagnosisSection = `
                            <div class="border-b pb-4">
                                <p class="text-sm text-gray-500">Belum ada hasil diagnosis yang tersedia.</p>
                            </div>
                        `;
                    }
                    ...
                    let content = `
                        <div class="space-y-4">
                            <div class="border-b pb-4">
                                <h4 class="font-semibold text-gray-900 mb-3">Informasi Dokter</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Dokter</p>
                                        <p class="mt-1 text-sm text-gray-900">${data.doctor?.name || '-'}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Spesialisasi</p>
                            ...
                            <div class="border-b pb-4">
                                <h4 class="font-semibold text-gray-900 mb-3">Detail Konsultasi</h4>
                                <div>
                            ...
                            <div>
                                <h4 class="font-semibold text-gray-900 mb-3">Status Permintaan</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">${new Date(data.created_at).toLocaleDateString('id-ID')}</p>
                                    </div>
                                    ${data.scheduled_date ? `
                                    <div>
